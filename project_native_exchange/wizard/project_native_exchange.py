...
class ProjectNativeExchangeImport(models.TransientModel):
    ...
    import_task_line_ids = fields.One2many('project.native.exchange.import.task.line', 'import_id', string='Task Lines')
    ...
    def func_mapper(self, value, data):
        ...
        if value["func"] == "iso8601_float":
            data_float = exchange_tool.xm_iso8601(value=data, direct="from_xml", type_con="float")
            return data_float

        if value["func"] == "iso8601_int":
            data_integer = exchange_tool.xm_iso8601(value=data, direct="from_xml", type_con="int")
            return data_integer
    ...
    def parse_task(self, root, nsmap, xml_import=False, project_id=False):
        ...
        xml_parse_task_fields = {
            ...
            "0_OutlineNumber":  {"xml": "OutlineNumber",            "func": "int"},
            ...
            "plan_duration":    {"xml": "Work",                     "func": "iso8601_int"},
            "native_duration":  {"xml": "Duration",                 "func": "iso8601_int"},
        ...
        for project_task in project_tasks_list:
            ...
            if o_uid != 0:
                ...
                # Not need for now, but still need for future
                ...
                # dt_date_start = project_task["date_start"]
                # dt_date_end = project_task["date_end"]
                ...
                task_id = task_obj.id
                project_task["odoo_uid"] = task_id
                project_task_id_uid[o_uid] = task_id

        # build parent-child relations
        parent_stack = []
        for i, task in enumerate(project_tasks_list):
            if i > 0:
                if task["sorting_level"] > project_tasks_list[i - 1]["sorting_level"]:
                    parent_stack.append(i-1)
                elif task["sorting_level"] < project_tasks_list[i - 1]["sorting_level"]:
                    while parent_stack and project_tasks_list[parent_stack[-1]]["sorting_level"] >= task["sorting_level"]:
                        parent_stack.pop()
            task["parent"] = project_tasks_list[parent_stack[-1]]["odoo_uid"] if parent_stack else None

        # write parent-child relations
        for project_task in project_tasks_list:
            task_id = project_task["odoo_uid"]
            task_obj = project_task_model.browse(task_id)

            if project_task["parent"]:
                parent_id = project_task["parent"]
                task_obj.write({
                    'parent_id': parent_id
                })
...
class ProjectNativeExchange(models.TransientModel):
    ...
    def get_data_xml(self, project_id):
        ...
        for p_task in p_tasks:
            ...
            t_duration = timedelta(seconds=p_task.native_duration)
            t_plan_duration = timedelta(seconds=p_task.plan_duration)

            iso8601duration = exchange_tool.to_iso8601(t_duration)
            iso8601plan_duration = exchange_tool.to_iso8601(t_plan_duration)
            ...
            ET.SubElement(task, "Duration").text = iso8601plan_duration #"PT72H0M0S"
            ...
            ET.SubElement(task, "RemainingDuration").text = iso8601duration  # "PT72H0M0S"
            ET.SubElement(task, "Work").text = iso8601plan_duration  # "PT72H0M0S"
