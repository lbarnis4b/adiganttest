...
class ProjectNativeExchangeTool(models.TransientModel):
    ...
    def iso8601_to(self, value):
        # split 1 step. PT8H0M0S - PT3632H30M00S
        hours = 0
        minutes = 0
        seconds = 0

        next_step = value.split("PT", 2)[-1] # ['', '8H0M0S']
        # hours
        if "H" in next_step:
            hours = int(next_step.split("H", 1)[0])
            next_step = next_step.split("H", 1)[-1] # ['0M0S']
        # minutes
        if "M" in next_step:
            minutes = int(next_step.split("M", 1)[0])
            next_step = next_step.split("M", 1)[-1] # ['0S']
        # seconds
        if "S" in next_step:
            seconds = int(next_step.split("S", 1)[0])
    ...
    def xm_iso8601(self, value=False, direct="to_xml", type_con=None):
        ...
        if direct == "from_xml":
            ...
            duration = self.iso8601_to(value)
